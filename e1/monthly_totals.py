...
def pivot_months_pandas(data):
    ...
    """
    # ...
    # No. 1 : Add a column 'month' that contains the results of applying the date_to_month function to the existing 'date' column
    data['month'] = data['date'].apply(date_to_month)
    
    # No. 2 : Use the Pandas groupby method to aggregate over the name and month columns. Sum each of the aggregated values to get the total
    # hint : grouped_data.aggregate({'precipitation': 'sum'}).reset_index()
    
    groupby_total = data.groupby(['name','month']).agg({'precipitation': 'sum'}).reset_index() # given site's example used .agg instead of aggregate
    
    # No. 3: Use the Pandas pivot method to create a row for each station (name) and column for each month.
    monthly = groupby_total.pivot(index = 'name', columns = 'month', values = 'precipitation')
    
    # No. 4 : Use the Pandas groupby method to aggregate over the name and month columns. Sum each of the aggregated values to get the count
    # hint : grouped_data.aggregate({'precipitation': 'count'}).reset_index()
    
    groupby_count = data.groupby(['name','month']).agg({'precipitation': 'count'}).reset_index() 
    
    # No. 5: Use the Pandas pivot method to create a row for each station (name) and column for each month.
    counts = groupby_count.pivot(index = 'name', columns = 'month', values = 'precipitation')
