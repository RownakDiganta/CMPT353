...
def get_data(filename):
    ...
    sysinfo[y_column] = sysinfo[y_column].shift(-1) # As it should be the temperature value from the next row (Followed hint)
...
def get_trained_coefficients(X_train, y_train):
    ...
    model = LinearRegression(fit_intercept= False).fit(X_train, y_train) # Taken inspiration from the hint's website 
