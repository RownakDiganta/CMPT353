...
import sys
# Importing all the libraries which was used in exercise 7 
...
def main():
    ...
    # Taken from exercise 7 and slides 
    # Partition your data into training and validation sets using train_test_split.
    X_train, X_valid, y_train, y_valid = train_test_split(X,y) 
    
    # creating 6 models in the same order as mentioned in the array models (Given)
    
    # First model : bayes_rgb_model
    bayes_rgb_model = GaussianNB()
    bayes_rgb_model.fit(X_train,y_train)
    
    # Second model: bayes_convert_model
    bayes_convert_model = make_pipeline(FunctionTransformer(rgb2lab),GaussianNB()) # by default validate in FunctionTransformer() is false  
    bayes_convert_model.fit(X_train,y_train)
    
    # Third model: knn_rgb_model
    knn_rgb_model = KNeighborsClassifier(n_neighbors= 8)
    knn_rgb_model.fit(X_train,y_train)
    
    # Fourth model: knn_convert_model
    knn_convert_model = make_pipeline(FunctionTransformer(rgb2lab),KNeighborsClassifier(n_neighbors= 8))
    knn_convert_model.fit(X_train,y_train)
    
    # Fifth model: rf_rgb_model
    rf_rgb_model = RandomForestClassifier(n_estimators= 100, max_depth= 16, min_samples_leaf = 6 )
    rf_rgb_model.fit(X_train,y_train)
    
    # Sixth model: rf_convert_model
    rf_convert_model = make_pipeline(FunctionTransformer(rgb2lab), RandomForestClassifier(n_estimators= 100, max_depth= 16, min_samples_leaf = 6 ))
